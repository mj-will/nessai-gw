...
def test_comoving_vol_init(comoving_vol_converter):
    ...
    with (
        patch(
            "nessai_gw.reparameterisations.distance_converters.cosmo.Planck15",
            return_value=MagicMock(),
        ),
        patch(
            "nessai_gw.reparameterisations.distance_converters.cosmo.Planck15.comoving_distance",
            side_effect=[dc_min, dc_max],
        ) as cd_mock,
        patch(
            "nessai_gw.reparameterisations.distance_converters.cosmo.Planck15.luminosity_distance",
            side_effect=[dl_array],
        ) as ld_mock,
        patch(
            "nessai_gw.reparameterisations.distance_converters.cosmo.z_at_value",
            side_effect=z_values,
        ),
        patch("numpy.linspace", return_value=dc_array) as linspace_mock,
        patch(
            "nessai_gw.reparameterisations.distance_converters.interpolate.splrep",
...
def test_comoving_vol_to_uniform(comoving_vol_converter):
    ...
    with patch(
        "nessai_gw.reparameterisations.distance_converters.interpolate.splev",
        return_value=200,
    ) as interp_mock:
