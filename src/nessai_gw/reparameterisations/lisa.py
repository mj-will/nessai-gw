...
class LISAExtrinsicSymmetry(Reparameterisation):
    ...
    def determine_parameter(self, known_parameters: frozenset, required=True) -> str:
    ...
    def phase_parameter(self, name: Union[str, None]):
        if name is None:
            name = self.determine_parameter(self.known_phase_parameters, required=False)
            if name is None:
                return
        if self.prior_bounds[name][0] != 0:
            raise RuntimeError
    ...
    def _fold(
        ...
        if self.phase_parameter:
            phase_num = np.digitize(x[self.phase_parameter], self.phase_bins) - 1
        else:
            phase_num = 0

        if self.include_mode_index:
            x_prime["mode_index"] = (long_num + (lat_num * 4)) + (8 * phase_num)
