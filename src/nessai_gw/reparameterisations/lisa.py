...
class LISAExtrinsicSymmetry(Reparameterisation):
    ...
    minimum_mode_weight :
        ...
        disable.
    mode_subset :
        Optional shorthand for restricting the sampled sky modes when mapping
        from X-prime to X. Supported values are:
        ``"full"`` for all sky modes,
        ``"mirror"`` for the identity and mirror branches,
        ``"antipodal"`` for the identity and antipodal branches,
        and ``"mirror-antipodal"`` for the four branches generated by the
        mirror and antipodal symmetries. If ``phase_fold`` is enabled, the
        selected sky modes are duplicated across both phase branches.
    allowed_mode_indices :
        Optional explicit list of allowed mode indices to sample when mapping
        from X-prime to X. If specified, this overrides ``mode_subset``.
    lambda_parameter :
    ...
    phase_fold :
        If True and a phase parameter is present, fold the phase modulo
        :math:`\\pi` and include the corresponding phase symmetry branch.
        If False, the phase parameter is copied through unchanged.
    rng :
    ...
    def __init__(
        ...
        mode_subset: Literal[
            "full", "mirror", "antipodal", "mirror-antipodal"
        ] = "full",
        allowed_mode_indices: Iterable[int] | None = None,
        ...
        self.phase_fold = phase_fold

        if self.phase_fold and self.phase_parameter is None:
            raise RuntimeError(
        ...
        self.allowed_mode_indices = self._configure_allowed_mode_indices(
            mode_subset=mode_subset,
            allowed_mode_indices=allowed_mode_indices,
        )
    ...
    @property
    def phase_fold(self) -> bool:
        return self._phase_fold

    @phase_fold.setter
    def phase_fold(self, value: bool) -> None:
        self._phase_fold = bool(value) and self.phase_parameter is not None
    ...
    def max_n_modes(self):
        ...
        Depending on the value of `phase_fold`, the number of modes will
        be either 8 or 16.
        """
        if self.phase_fold:
            return 16
    ...
    @property
    def n_modes(self):
        """Number of modes in the selected subset."""
        return self.allowed_mode_indices.size

    def _configure_allowed_mode_indices(
        self,
        mode_subset: str,
        allowed_mode_indices: Iterable[int] | None,
    ) -> np.ndarray:
        if allowed_mode_indices is not None:
            indices = np.asarray(list(allowed_mode_indices), dtype=int)
        else:
            try:
                sky_indices = np.asarray(
                    self.mode_subset_aliases[mode_subset], dtype=int
                )
            except KeyError as exc:
                valid = ", ".join(sorted(self.mode_subset_aliases))
                raise RuntimeError(
                    f"Unknown mode_subset: {mode_subset}. Valid options: {valid}"
                ) from exc
            if self.phase_fold:
                indices = np.concatenate([sky_indices, sky_indices + 8])
            else:
                indices = sky_indices

        if indices.ndim != 1 or indices.size == 0:
            raise RuntimeError(
                "allowed_mode_indices must be a non-empty 1D sequence"
            )
        if np.unique(indices).size != indices.size:
            raise RuntimeError(
                "allowed_mode_indices must not contain duplicates"
            )
        if np.any(indices < 0) or np.any(indices >= self.max_n_modes):
            raise RuntimeError(
                f"allowed_mode_indices must be between 0 and {self.max_n_modes - 1}"
            )
    ...
    def update(self, x):
        ...
        if self.estimate_mode_weights:
            ...
            counts = np.bincount(mode_ids.index, minlength=self.max_n_modes)
            mode_weights = counts[self.allowed_mode_indices] / counts.sum()
    ...
    def sample_mode_index(self, size: int) -> np.ndarray:
        ...
        if self.estimate_mode_weights:
            return self.rng.choice(
                self.allowed_mode_indices, size=size, p=self.mode_weights
            )
        else:
            return self.rng.choice(self.allowed_mode_indices, size=size)
    ...
    def unfold(
        ...
        if self.phase_fold:
            ...
            x[self.phase_parameter] = x_prime[self.phase_parameter_prime]
        return x, x_prime, log_j
